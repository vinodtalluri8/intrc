...
export class DropdownComponent implements OnInit {
  ...
  set datasource(data: SelectItem[]) {
    ...
    if (this.selectedItemData && data && data.length > this.size) {
      const index = this.originaldatasource.findIndex(item => item ? item.value === this.selectedItemData : item);
      if (index > this.size) {
        for (let i = (index - 200); this.filtereddatasource.length <= this.size; i++) {
          this.filtereddatasource.push(this.originaldatasource[i]);
        }
      }
    }
    if (data && data.length > this.size && this.filtereddatasource.length === 0) {
  ...
  @Input() set selectedItem(selectedItem: string) {
    console.log('dropdown sleected item', selectedItem);
